...
static bool summarize_parameter(const struct kfsw_param_info *info, void *context)
{
	...
	summary->all_read_only &= info->read_only;
...
ZTEST(boton_test, test_parameter_definition_set_has_stable_read_only_nonpersistent_ids)
{
	...
	zassert_equal(kfsw_boton_test_param_definitions.count, 2U);
...
ZTEST(boton_test, test_parameter_visit_contains_both_module_values)
{
